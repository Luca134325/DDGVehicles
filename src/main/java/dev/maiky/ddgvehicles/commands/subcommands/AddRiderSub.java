...
public class AddRiderSub extends ISub {

    public AddRiderSub(VehicleCommand command) {
        super(command);
    }
    ...
    public void fire(CommandSender sender, String[] args, String label) {
        ...
        boolean add = manager.addRider(license, offlinePlayer.getUniqueId());

        if (add) {
            player.sendMessage("§3Je hebt §b" + offlinePlayer.getName() + "§3 toegevoegd als bestuurder van dit voertuig.");
        }else{
            player.sendMessage("§cDe speler §4" + offlinePlayer.getName() + "§c is al een bestuurder van dit voertuig.");
        }
    }
