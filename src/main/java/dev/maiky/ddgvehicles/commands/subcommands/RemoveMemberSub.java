...
public class RemoveMemberSub extends ISub {

    public RemoveMemberSub(VehicleCommand command) {
        super(command);
    }
    ...
    public void fire(CommandSender sender, String[] args, String label) {
        ...
        boolean add = manager.removeMember(license, offlinePlayer.getUniqueId());

        if (add) {
            player.sendMessage("§3Je hebt §b" + offlinePlayer.getName() + "§3 verwijderd als passagier van dit voertuig.");
        }else{
            player.sendMessage("§cDe speler §4" + offlinePlayer.getName() + "§c is geen  passagier van dit voertuig.");
        }
    }
