...
public class AddMemberSub extends ISub {

    public AddMemberSub(VehicleCommand command) {
        super(command);
    }
    ...
    public void fire(CommandSender sender, String[] args, String label) {
        ...
        boolean add = manager.addMember(license, offlinePlayer.getUniqueId());

        if (add) {
            player.sendMessage("§3Je hebt §b" + offlinePlayer.getName() + "§3 toegevoegd als passagier van dit voertuig.");
        }else{
            player.sendMessage("§cDe speler §4" + offlinePlayer.getName() + "§c is al een passagier van dit voertuig.");
        }
    }
